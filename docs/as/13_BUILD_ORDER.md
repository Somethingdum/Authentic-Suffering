...
### P12 — Surfaces
Read: CHEATS.md, 09 §8, 10 §2.6, `cheats/*`, `service/voice.py`, `content/importers.py`, `service/death.py`, `service/runs.py` (worlds).
1. BUILT (D-100; tests 12 §3.18): `cheats/commands.py` (activate, parse, resolve, execute, the persona, god mode in `physical.bodies.apply_harm`, the standing brief at S4) + GameService routing + the guide's deflection + the `cheat_admin` pack. 1b. The owner's additions (D-78, D-79: will overwrite, memory wipe, remote infection, horde and census commands, the cure — BUILT, D-101), Willis and the Wild Card (BUILT, D-102: the code box, the reality exception, the wonders, the endless plate, TEMPER-10, REL-06, WORLD-07). 1c. Plain-words cheats and the Act / Say / Cheat input (BUILT, D-103: `cheats/interpret.py`, forced acts, blasts, shows, `turn_compose`, `PlayView.say_to` / `console`; the Play screen's input fields are part of the P8 screen). 1d. The seal (BUILT, D-104: `lanes/seal.py`, 02 §9, SEAL-01..05 — nothing leaves the owner's machines). 2. `content/importers.py` (files, character cards, docx, dossier intake). 3. `service/death.py` (the death screen, the reveal and Willis at every death: BUILT, D-105; a new life here, RUN-07, still to do). 3b. The Doom and the Voice (BUILT, D-106, deepened D-107: the screaming, the broken mind, the bitten doom, despair; tests 12 §3.22; 3c. Parkour (BUILT, D-108: height, roofs, climbs, gaps, edges, falls, runners that follow; tests 12 §3.23); tests 12 §3.21: `physical.bodies` DOOM-01..05 and the dooms table, `service/voice.py`, the pipeline's Doom scene, `turn.intake` DOOM-07, the loan and Willis collecting, the Doom overlay). 4. Worlds (list/export/import, `create_run(world_id=…)`). 5. Migration framework (`kernel/migrations/`, consent flow). 6. Read aloud via Talemate TTS (a local voice only, its model fetched by `tools/as/setup.py` at install — the sealed game never downloads, D-104).
