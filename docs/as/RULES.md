...
| Id | Statement | Stated in | Enforced in | Tested by |
|---|---|---|---|---|
| CHEAT-01 | `2508` activates only as a standalone number; the activating line is consumed; before activation no `/command` parses (it is ordinary input) | CHEATS §9 | `as_engine/cheats/__init__.py`, `as_engine/cheats/commands.py`, `as_engine/service/game_service.py` | `contract/p12_surfaces/test_cheat_console.py`, `contract/p12_surfaces/test_cheat_protocol.py` |
| CHEAT-02 | A Sandbox twin's world invariants equal the clean run's once cheat entities are excluded | CHEATS §9 | `as_engine/audit/abuse.py`, `as_engine/cheats/__init__.py`, `as_engine/cheats/commands.py` | `contract/p12_surfaces/test_cheat_twin.py` |
...
| CHEAT-10 | `cheat_*` packs are invisible to worldgen and the wizard; only /spawn reads them | CHEATS §9 | `as_engine/cheats/_impl_cheats.py`, `as_engine/cheats/commands.py`, `as_engine/content/pack.py`, `as_engine/service/game_service.py`, `as_engine/service/runs.py`, `as_engine/world/worldgen/people.py`, `as_content/packs/cheat_admin/pack.yaml` | `contract/p12_surfaces/test_cheat_console.py`, `contract/p12_surfaces/test_willis.py` |
| CHEAT-11 | A `standing_brief` actor holds the brief's truths every turn it is HOT/WARM, every belief arriving through perception.grant with provenance cheat; the tag outside a `cheat_` pack is a content error | CHEATS §9 | `as_engine/cheats/commands.py`, `as_engine/content/pack.py`, `as_engine/mind/perception.py`, `as_engine/turn/pipeline.py`, `as_content/packs/cheat_admin/actors/fredrick.yaml` | `contract/p12_surfaces/test_cheat_protocol.py` |
| CHEAT-12 | The code box answers only accepted or not; after the code the New Life list shows the `cheat_` packs' characters and a life as one opens with the console active and the run a Sandbox; before it they are characters nobod… | CHEATS §9 | `as_engine/cheats/commands.py`, `as_engine/contracts/protocol.py`, `as_engine/service/game_service.py`, `as_engine/service/runs.py`, `as_engine/world/worldgen/opening.py`, `as_content/packs/cheat_admin/pack.yaml`, `as_content/packs/cheat_admin/pcs/willis.yaml` | `contract/p10_world/test_new_life.py`, `contract/p12_surfaces/test_willis.py` |
...
| LOOK-06 | LOOK-06, SMELL-04); **Your | 05_ACTORS §3 | `as_engine/mind/_impl_packet.py`, `as_engine/mind/packet.py`, `as_engine/mind/perception.py`, `as_engine/narration/location.py`, `as_engine/narration/narrator.py` | `contract/p04_one_actor/test_appearance_in_packet.py` |
...
| LOOK-10 | LOOK-10 (the owner's F1a-2: everyone has a visual identity, and people dress for where they live) generated_looks(seed) -> dict: a contracts.dossier.Looks as a dict, a pure function of the PersonSeed (implemented below)… | as_engine/world/worldgen/people.py | `as_engine/society/population.py`, `as_engine/world/worldgen/people.py` | `contract/p10_world/test_materialise.py` |

## LOOP

| Id | Statement | Stated in | Enforced in | Tested by |
|---|---|---|---|---|
| LOOP-01 | LOOP-01 open_loop(tx, holder_id, kind, text, subject_ids, strength, cause, at, turn_index, due_at=None, links=()) -> loop_id (kind 'olp'). ``kind`` must be an OpenLoopKind value, ``text`` non-empty after strip, ``streng… | as_engine/mind/mind.py | `as_engine/mind/mind.py` | `contract/p06_memory/test_mind.py` |
...
| MEM-14 | MEM-14 episodes: the holder's episodes with decayed 0 and (B5, MEM-18) quarantined 0. Anchors (anchor 1) come first and always: ordered (salience desc, at desc, episode_id), at most 2. Then the rest by score = salience… | as_engine/mind/retrieval.py | `as_engine/mind/memory.py`, `as_engine/mind/retrieval.py` | `contract/p06_memory/test_memory_v2.py`, `contract/p06_memory/test_retrieval.py` |
...
| Id | Statement | Stated in | Enforced in | Tested by |
|---|---|---|---|---|
| PROM-01 | PROM-01 CATEGORIES and STATUSES below. A status moves only along NEXT: proposed -> understood, accepted, withdrawn understood -> accepted, in_progress, fulfilled, failed, withdrawn accepted -> in_progress, fulfilled, fa… | as_engine/mind/promise.py | `as_engine/mind/promise.py` | `contract/p06_memory/test_promises.py` |
| PROM-02 | PROM-02 hold(tx, holder_id, *, promiser_id, promisee_id, category, text, object_id, condition, source_event_id, loop_id, status, at, turn_index) -> promise_id (kind 'prm'). The holder's understanding of a promise made i… | as_engine/mind/promise.py | `as_engine/mind/memory.py`, `as_engine/mind/promise.py` | `contract/p06_memory/test_promises.py` |
| PROM-03 | PROM-03 agreement: after a hold, when the promiser and the promisee each hold an understanding of the same source_event_id with the same promiser and promisee (so the words were said by one and heard by the other, PROM-… | as_engine/mind/promise.py | `as_engine/mind/promise.py` | `contract/p06_memory/test_promises.py` |
| PROM-04 | PROM-04 set_status(tx, promise_id, status, cause, at, turn_index) -> Event / None: the promise's current status -> None, nothing committed. A move NEXT does not allow -> ValueError. Else PROMISE_STATUS {promise_id, hold… | as_engine/mind/promise.py | `as_engine/mind/promise.py` | `contract/p06_memory/test_promises.py` |
| PROM-05 | PROM-05 on_loop_closed(tx, loop_id, status, cause, at, turn_index) -> list[Event] — mind.mind close_loop calls it after committing the loop's own event. The holder's promise carried by that loop (promises.loop_id; none… | as_engine/mind/promise.py | `as_engine/mind/promise.py` | `contract/p06_memory/test_promises.py` |
| PROM-06 | PROM-06 disputed: when that promise has an agreement and the other party's promise of the same agreement is 'fulfilled' while this one is now 'failed', or the other way round, both become 'disputed' — the promiser's PRO… | as_engine/mind/promise.py | `as_engine/mind/promise.py` | `contract/p06_memory/test_promises.py` |
| PROM-07 | PROM-07 suffix(store, loop_id) -> str: how a promise the loop carries stands, for the packet line of that loop (mind.packet open_loops): '' when the loop carries no promise or its status is terminal; else SUFFIX_WORDS[s… | as_engine/mind/promise.py | `as_engine/mind/promise.py` | `contract/p06_memory/test_promises.py` |
...
| REL-06 | REL-06 report.soak = {days, world_days: the WORLD_DAY events the soak committed, events: every event it committed, and the count of each of DEATH, OFFSCREEN_DEATH, BIRTH, RAID, HORDE_FORMED, SHORTAGE, RUMOUR_SPREAD, FAC… | as_engine/audit/release.py | `as_engine/audit/release.py`, `as_engine/cheats/commands.py`, `as_engine/mind/mind.py`, `as_engine/world/worldgen/opening.py`, `as_content/packs/cheat_admin/pcs/willis.yaml` | `contract/p12_surfaces/test_willis.py` |
...
| STORE-12 | STORE-12 (fidelity C10, Actor v2 B5c) ``event.links`` — the causes besides the primary parent ``cause_event_id`` — are checked before anything is written: every link's event_id names an event already in the log (committ… | as_engine/kernel/store.py | `as_engine/contracts/events.py`, `as_engine/kernel/events.py`, `as_engine/kernel/store.py`, `as_engine/mind/mind.py`, `as_engine/physical/bodies.py` | `contract/p00_substrate/test_event_links.py`, `contract/p07_slice/test_answers.py` |
...
| WG-26 | WG-26 Pack actors. The canon actor records (not PCs), by ref — never one from a 'cheat_' pack (CHEAT-10: a life as a cheat_ pack's character has that pack in its canon, and its other people stay in it until /spawn; D-10… | as_engine/world/worldgen/people.py | `as_engine/world/worldgen/people.py`, `as_engine/world/worldgen/pipeline.py` | `contract/p10_world/test_worldgen_pipeline.py` |
...
| WILL-01 | *The Request Firewall (P4). Rules WILL-00..13, L6, L7. docs/as/05_ACTORS.md §Firewall.* | as_engine/mind/firewall.py | `as_engine/mind/firewall.py` | `contract/p04_one_actor/test_firewall.py` |
| WILL-02 | *The Request Firewall (P4). Rules WILL-00..13, L6, L7. docs/as/05_ACTORS.md §Firewall.* | as_engine/mind/firewall.py | `as_engine/mind/firewall.py` | `contract/p04_one_actor/test_firewall.py` |
...
| WILL-04 | *The Request Firewall (P4). Rules WILL-00..13, L6, L7. docs/as/05_ACTORS.md §Firewall.* | as_engine/mind/firewall.py | `as_engine/mind/firewall.py`, `as_engine/turn/cognition.py` | `contract/p04_one_actor/test_firewall.py`, `contract/p07_slice/test_slice_refusal.py` |
...
| WILL-08 | *The Request Firewall (P4). Rules WILL-00..13, L6, L7. docs/as/05_ACTORS.md §Firewall.* | as_engine/mind/firewall.py | `as_engine/mind/firewall.py`, `as_engine/turn/cognition.py` | `contract/p04_one_actor/test_firewall.py`, `contract/p07_slice/test_slice_refusal.py` |
| WILL-09 | *The Request Firewall (P4). Rules WILL-00..13, L6, L7. docs/as/05_ACTORS.md §Firewall.* | as_engine/mind/firewall.py | `as_engine/mind/firewall.py`, `as_engine/turn/cognition.py` | `contract/p04_one_actor/test_firewall.py`, `contract/p07_slice/test_answers.py` |
| WILL-10 | *The Request Firewall (P4). Rules WILL-00..13, L6, L7. docs/as/05_ACTORS.md §Firewall.* | as_engine/mind/firewall.py | `as_engine/mind/firewall.py`, `as_engine/mind/perception.py`, `as_engine/turn/cognition.py` | `contract/p04_one_actor/test_firewall.py` |
| WILL-11 | WILL-11 record_lie(tx, liar_id, to_id, signature, words, speech_event_id, at, turn_index) -> Event LIE_TOLD {liar_id, to_id, signature, words} (writer 'mind.mind', actor_id = liar_id, cause_event_id = the liar's SPEECH… | as_engine/mind/firewall.py | `as_engine/mind/firewall.py` | `contract/p06_memory/test_refusals.py` |
| WILL-12 | WILL-12 (AC09, Actor Spec §10: asking again is never better, but a person can reconsider) revise_refusal(tx, actor_id, requester_id, signature, at, turn_index, cause_event_id) -> Event / None: the refusal row of (actor_… | as_engine/mind/firewall.py | `as_engine/mind/firewall.py`, `as_engine/turn/cognition.py` | `contract/p06_memory/test_refusals.py`, `contract/p07_slice/test_answers.py` |
| WILL-13 | WILL-13 (AC09) record_unmet_assent(tx, actor_id, to_id, signature, words, chosen_def_id, speech_event_id, at, turn_index, ask_event_id=None) -> Event: ASSENT_UNMET {actor_id, to_id, signature, words, chosen_def_id} (wri… | as_engine/mind/firewall.py | `as_engine/mind/firewall.py` | `contract/p06_memory/test_refusals.py`, `contract/p07_slice/test_answers.py` |
...
| WORLD-07 | WORLD-07 (P12, D-102) The Wild Card is never where you left him: day() step 4b moves him each world day he is not in the active area. | as_engine/world/worldmove.py | `as_engine/world/worldgen/opening.py`, `as_engine/world/worldmove.py` | `contract/p12_surfaces/test_willis.py` |
